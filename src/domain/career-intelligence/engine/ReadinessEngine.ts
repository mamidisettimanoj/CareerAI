...
export function calculateReadiness(input: CareerIntelligenceInput): ReadinessScore {
  ...
  if (input.assessments.technicalScore === undefined && input.skills.length === 0) {
    ...
    // Estimate based on average of skill proficiency if no explicit assessment
    const validSkills = input.skills.filter(s => typeof s.proficiency === 'number');
    if (validSkills.length > 0) {
      const sum = validSkills.reduce((acc, s) => acc + (s.proficiency as number), 0);
      techScoreRaw = sum / validSkills.length;
    } else {
      techScoreRaw = 0;
    }
