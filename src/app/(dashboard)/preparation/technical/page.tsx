...
export default function TechnicalCenter() {
  const attempts = useLiveQuery(() => db.technicalAttempts.toArray()) || [];
  
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedOption, setSelectedOption] = useState<string | null>(null);
  const [showExplanation, setShowExplanation] = useState(false);

  const question = TECHNICAL_QUESTIONS[currentIndex];
  ...
  const handleSubmit = async () => {
    ...
    await db.technicalAttempts.put(attempt);
  ...
  const handleNext = () => {
    ...
    if (currentIndex < TECHNICAL_QUESTIONS.length - 1) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-heading font-bold flex items-center gap-2">
            <BookOpen className="h-6 w-6 text-orange-500" /> Technical CS Practice
          </h1>
      ...
      <Card className="border-2">
        ...
        <CardFooter className="bg-muted/20 border-t py-4 flex justify-between">
          <div className="text-sm text-muted-foreground">Question {currentIndex + 1} of {TECHNICAL_QUESTIONS.length}</div>
          {!showExplanation ? (
