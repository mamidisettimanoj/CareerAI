...
import { ArrowLeft, CheckCircle2, AlertTriangle, Bot, RefreshCw, XCircle, Target, CalendarDays, ListTodo } from 'lucide-react';
import { repositories } from '@/services/ServiceLocator';
import { careerService } from '@/services/CareerService';
import { UserProfile, IntelligenceResult } from '@/types';
...
export default function Result() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      let er = await careerService.getCachedAnalysis();
      if (!er) {
        er = await careerService.generateAnalysis();
      }
      setEngineResult(er);
      setLoading(false);
    }
  ...
  if (loading || !profile || !engineResult) {
    return <div className="flex items-center justify-center min-h-[40vh] text-muted-foreground">Loading analysis...</div>;
  }

  const score = engineResult.readiness.overallScore;
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" className="w-full sm:w-auto" onClick={handleRefresh} disabled={refreshing}>
            <RefreshCw className={`mr-2 h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} /> Refresh Analysis
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Main Score Card */}
        <Card className="lg:col-span-1 flex flex-col items-center justify-center text-center p-6 border-primary/20 relative overflow-hidden">
          ...
          <p className="text-base font-bold text-foreground">{profile.targetRole}</p>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Academic</span>
                  <span className="font-medium">{engineResult.readiness.dimensions.academic.score}%</span>
                </div>
                <Progress value={engineResult.readiness.dimensions.academic.score} className="h-2" />
              </div>
              
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Aptitude</span>
                  <span className="font-medium">{profile.skills.employabilityScore}%</span>
                </div>
                <Progress value={profile.skills.employabilityScore} className="h-2" />
              </div>

              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Technical Skills</span>
                  <span className="font-medium">{engineResult.readiness.dimensions.technical.score}%</span>
                </div>
                <Progress value={engineResult.readiness.dimensions.technical.score} className="h-2" />
              </div>

              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Communication</span>
                  <span className="font-medium">{profile.skills.communicationScore}%</span>
                </div>
                <Progress value={profile.skills.communicationScore} className="h-2" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-6 border-t border-border/50 mt-4">
              <div className="flex flex-col">
                ...
                <span className="text-lg font-medium">{profile.skills.projectsCount} / {profile.degree.internships}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-muted-foreground mb-1">Active Backlogs</span>
                <span className={`text-lg font-medium ${profile.degree.backlogs > 0 ? 'text-destructive' : 'text-success'}`}>
                  {profile.degree.backlogs}
                </span>
              </div>
            </div>
          </CardContent>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="border-success/20">
              ...
              <CardContent>
                <ul className="space-y-2">
                  {engineResult.readiness.topStrengths.map((strength, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      <span className="text-success mt-1">•</span> {strength}
                    </li>
                  ))}
                  {engineResult.readiness.topStrengths.length === 0 && (
            ...
            <Card className="border-destructive/20">
              ...
              <CardContent className="space-y-4">
                {engineResult.readiness.priorityImprovements.map((imp, i) => (
                ...
                {engineResult.readiness.priorityImprovements.length === 0 && (
          ...
          <Card >
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <Target className="h-5 w-5 text-primary" /> Target Role Match: {profile.targetRole}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {engineResult.roleMatch ? (
                <>
                  <div className="flex items-center gap-4 mb-6">
                    <div className="text-3xl font-bold text-primary">{engineResult.roleMatch.matchScore}%</div>
                    <Progress value={engineResult.roleMatch.matchScore} className="h-3 flex-1" />
                  </div>
                  <div className="text-sm text-muted-foreground mb-4">{engineResult.roleMatch.recommendation}</div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {engineResult.roleMatch.skillGaps.matchedSkills.map((skill, i) => (
                      <div key={'m-'+i} className="flex items-center justify-between p-2 rounded bg-card/50 border border-border/40 text-sm">
                        <span>{skill}</span>
                        <CheckCircle2 className="h-4 w-4 text-success" />
                      </div>
                    ))}
                    {engineResult.roleMatch.skillGaps.missingSkills.map((skill, i) => (
                      <div key={'miss-'+i} className="flex items-center justify-between p-2 rounded bg-card/50 border border-border/40 text-sm">
                        <span>{skill}</span>
                ...
                <div className="text-muted-foreground text-sm">Role match data not available.</div>
              )}
            </CardContent>
          </Card>
