      ...
      <div className="container relative z-10 px-4 md:px-6 flex flex-col items-center text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-16 w-full sm:w-auto px-4">
          <Link to="/predict" className="w-full sm:w-auto">
            <Button size="lg" className="w-full font-semibold bg-primary hover:bg-primary/90 glow-primary h-14 sm:h-12 px-8">
              Analyze My Profile
              <ArrowRight className="ml-2 h-5 w-5 sm:h-4 sm:w-4" />
            </Button>
          </Link>
          <a href="#explore-tools" className="w-full sm:w-auto">
            <Button variant="outline" size="lg" className="w-full font-semibold h-14 sm:h-12 px-8 glass-panel-hover">
        ...
        <div id="explore-tools" className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 pt-8">
          {TOOLS.map((tool, idx) => (
            <Link key={idx} to={tool.href} className="block group">
              <div className="glass-panel p-6 flex flex-col items-center text-center h-full glass-panel-hover transition-all duration-300">
